...
noxp:set_add_func(function(data, modifier_index)
    ...
    InstanceExtManager.add_callback(data.skill.parent, "pre_player_level_up", data:get_id(), function (player)
        return false
    end)
end)
noxp:set_remove_func(function (data, modifier_index)
    InstanceExtManager.remove_callback(data.skill.parent, "pre_player_level_up", data:get_id())
end)
