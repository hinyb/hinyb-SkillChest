...
noxp:set_add_func(function(data, modifier_index)
    ...
    Instance_ext.add_callback(data.skill.parent, "pre_player_level_up", data:get_id(modifier_index), function (player)
        return false
    end)
end)
noxp:set_remove_func(function (data, modifier_index)
    Instance_ext.remove_callback(data.skill.parent, "pre_player_level_up", data:get_id(modifier_index))
end)
